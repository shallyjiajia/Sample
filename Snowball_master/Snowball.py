...
def Load_Seed(seed,name):
    #每种关系生成对应的txt文件
    f1 = open('seeds_positive' +'.txt' ,'w')
    f2 = open('seeds_negative'+'.txt','w')
    f1.write('e1:'+name[0]+' \n')
    f1.write('e2:'+name[1]+' \n')
    f1.write('\n')
    f2.write('e1:'+name[0]+' \n')
    ...
    for k in range(len(seed)):
        f1.write(seed[name[2]][k]+';'+seed[name[3]][k]+'\n')
    f2.write(seed[name[2]][0]+';'+seed[name[3]][0]+'\n')
    f1.close()
    f2.close()
    """
    生成训练所用的txt文件，并调用snowball算法 
    """
...
def relationships(data, types, name):
    org_loc = []
    org = []
    loc = []
    ...
    f_data[name1] = org
    f_data[name2] = loc
    f_data['relationship'] = types*len(org)
    file_name = types+'.csv'
    file_path = '../data'
    path = os.path.join(file_path,file_name)
    f_data.to_csv(path, index=False)

    return org, loc, org_loc, f_data
...
if __name__=='__main__':
    ...
    for name in relationship_type:
        temp = pd.read_excel('../data/Seed关系库.xlsx',sheet_name=name)
        seed_relationship.append(temp)
    for i in range(len(seed_relationship)):
        print("开始训练"+relationship_type[i]+'关系')
        seed=seed_relationship[i]
        name= entity[i]
        Load_Seed(seed,name)
        main()
        print(relationship_type[i] + '关系训练完成')
        #将reationships.txt转成csv关系格式
        data = 'relationships.txt'
        relationships(data, relationship_type[i],entity[i])
